...
#     ../build/results/exN-your_stdout.txt - standard output of your project
#     ../build/results/exN-your_stderr.txt - standard error output of your project
#     ../build/results/exN-ref_stdout.txt  - what the stdout should look like
#     ../build/results/exN-diff_out.txt    - diffence of your stdout and the ref stdout
#
# !! WARNING !!
...
test_example() {
    ...
    ret=$?
    if [ $ret -ne 0 ]; then
        echo -e "      FAILED (see $RESULTS_PATH/ex$exampleN-diff_out.txt)\n"
        echo "$diffOut" > $RESULTS_PATH/ex$exampleN-diff_out.txt 
    else
        echo -e "      OK\n"
    fi
}
...
# deleting something we're not supposed to (e.g. if a student uses ../build/inputs
# for their own tests)
if [ -d "$INPUTS_PATH" ] || [ -d "$RESULTS_PATH" ]; then
    echo "Warning: Directories \"$INPUTS_PATH\" and \"$RESULTS_PATH\" will now be deleted!"
    # echo "         Press enter to continue. Or press CTRL+C to stop the script."
    # read # wait for confirmation
    rm -rf $INPUTS_PATH $RESULTS_PATH
fi
...
echo -ne "$MAZE"           > $INPUTS_PATH/bludiste.txt
echo -ne "$MAZE_INVALID"   > $INPUTS_PATH/bludiste_invalid.txt
echo -ne "$REF_STDOUT_EX1" > $RESULTS_PATH/ex1-ref_stdout.txt
echo -ne "$REF_STDOUT_EX2" > $RESULTS_PATH/ex2-ref_stdout.txt
echo -ne "$REF_STDOUT_EX3" > $RESULTS_PATH/ex3-ref_stdout.txt
echo -ne "$REF_STDOUT_EX4" > $RESULTS_PATH/ex4-ref_stdout.txt
echo -ne "$REF_STDOUT_EX5" > $RESULTS_PATH/ex5-ref_stdout.txt
echo -ne "$REF_STDOUT_EX6" > $RESULTS_PATH/ex6-ref_stdout.txt
echo -ne "$REF_STDOUT_EX7" > $RESULTS_PATH/ex7-ref_stdout.txt

# test examples from the assignment
test_example "--test         $INPUTS_PATH/bludiste.txt"         1
test_example "--test         $INPUTS_PATH/bludiste_invalid.txt" 2
test_example "--lpath    6 1 $INPUTS_PATH/bludiste.txt"         3
test_example "--lpath    6 7 $INPUTS_PATH/bludiste.txt"         4
test_example "--rpath    6 1 $INPUTS_PATH/bludiste.txt"         5
test_example "--rpath    6 7 $INPUTS_PATH/bludiste.txt"         6
test_example "--shortest 3 7 $INPUTS_PATH/bludiste.txt"         7
